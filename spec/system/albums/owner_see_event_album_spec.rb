...
describe "owner see an album from a event" do
  ...
  it "and do not have the option to add images from other's events" do
    ...
    buffet2 = Buffet.create!(name: 'Dedi', corporate_name: 'Débora Gourmet alimentos', 
                          register_number: '12443456000145', phone: '53 991535353', email: 'dedi@delicias.com',
                          address: 'Andrade Neves, 15', district: 'Centro', state: 'RS', city: 'Piratini',
                          payment_method: 'Pix, Dinheiro', description: 'Qualidade e excelência para melhor servi-lo')
    user2.update!(buffet_id: buffet2.id)
    event2 = Event.create!(name: 'Formatura', description: 'Solenidade elegante para recepções de colação de grau', min_qtd: 50, max_qtd: 90,
                            duration: 250, menu: 'Canapés', buffet_id: buffet2.id)
    price = Price.create!(base_price: 5000, sp_base_price:6000, sp_additional_person:500,
