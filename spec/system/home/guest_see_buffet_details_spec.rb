...
describe 'Guest sees a details from buffet' do
  it 'and see disponible events' do
    ...
    event2 = Event.create!(name: 'Formatura',
      ...
      min_qtd: 20, max_qtd: 40, duration: 250, menu: 'Canapés',
      buffet_id: buffet.id)
    price2 = Price.create!(base_price: 3000, sp_base_price:4000,
      sp_additional_person:200, additional_person:250, sp_extra_hour:10,
      extra_hour:5, event_id: event2.id)
    ...
    expect(page).to have_content 'Capacidade Máxima: 40'
    expect(page).to have_content 'A partir de: R$ 3000,00'
  end
end