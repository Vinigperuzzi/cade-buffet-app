...
describe 'User access chat' do
  ...
  it "and access chat page for only it's own order's chat" do
    ...
    event2 = Event.create!(name: 'Formatura', description: 'Casamento de alto padrão', min_qtd: 200, max_qtd: 400,
                            duration: 250, menu: 'Doces, tortas, bolos e canapés', buffet_id: buffet2.id)
    price2 = Price.create!(base_price: 10000, sp_base_price:11000, sp_additional_person:1500,
                          additional_person:1000, sp_extra_hour:300, extra_hour:200, event_id: event2.id)
    ...
    order2 = Order.create!(buffet_id: buffet2.id, event_id: event2.id, event_date: Date.new(2134, 5, 1), estimated_qtd: 350,
                          event_details: 'A festa deve ser realizada duranto do dia caso não esteja chovendo', address: '', order_status: :waiting,
                          final_price: 150000, extra_tax: 0, discount: 0, customer_id: customer.id, out_doors: false)
  ...
  it "and can see messages send by customer and answer" do
    ...
    order = Order.create!(buffet_id: buffet.id, event_id: event.id, event_date: Date.new(2134, 5, 1), estimated_qtd: 30,
                          event_details: 'Esse evento deve possuir toda a temática com corações rosa', address: '', order_status: :waiting,
                          final_price: 11000, extra_tax: 1000, discount: 500, customer_id: customer.id, out_doors: false)
    message = Message.create!(order_id: order.id, sender: :customer, customer_read: false, user_read: false, message_text: 'Olá, tudo bem?')
  ...
  it "and can see messages marked as not visualized" do
    ...
    order = Order.create!(buffet_id: buffet.id, event_id: event.id, event_date: Date.new(2134, 5, 1), estimated_qtd: 30,
                          event_details: 'Esse evento deve possuir toda a temática com corações rosa', address: '', order_status: :waiting,
                          final_price: 11000, extra_tax: 1000, discount: 500, customer_id: customer.id, out_doors: false)
    message = Message.create!(order_id: order.id, sender: :user, customer_read: false, user_read: true, message_text: 'Olá, tudo bem?')
