...
describe "Customer see order's list" do
  it "and only see self orders" do
    ...
    customer2 = Customer.create!(email: 'customer@email.com', password: 'password2',
      name: 'cliente2', cpf: '283.101.870-68')
  ...
  it "and cannot force url for other customer's order's details" do
    ...
    customer2 = Customer.create!(email: 'cliente2@email.com', password: 'password2',
      name: 'cliente2', cpf: '880.275.650-30')
    ...
    order1 = Order.create!(buffet_id: buffet.id, event_id: event.id,
      ...
      discount: nil, customer_id: customer.id, out_doors: false)
    order1.valid?
    order2 = Order.create!(buffet_id: buffet.id, event_id: event.id,
      event_date: 1.day.from_now, estimated_qtd: 35,
      event_details: 'Quero temática Geek para a formatura em Ciência da Computação',
      address: '', order_status: :waiting, final_price: nil, extra_tax: nil,
      discount: nil, customer_id: customer2.id, out_doors: false)
    order2.valid?
  ...
  it "and see details from any pending order" do
    ...
    order = Order.create!(buffet_id: buffet.id, event_id: event.id,
      ...
      event_details: 'Esse evento deve possuir toda a temática com corações rosa',
      address: '', order_status: :waiting, final_price: nil, extra_tax: 1000,
    ...
    formatted_date = I18n.localize(order.event_date.to_date)
  ...
  it "and see details from any evaluated order" do
    user = User.create!(email: 'vinicius@email.com', password: 'password')
    buffet = Buffet.create!(name: 'Vini', corporate_name: 'Vinícius Gourmet alimentos', 
      register_number: '12456456000145', phone: '53 991814646',
      email: 'vinigperuzzi@gourmet.com', address: 'Estrada do Laranjal, 695',
    ...
    expect(page).to have_content "Código: #{order.code}"
    expect(page).to have_content 'deve ser realizado no endereço: No próprio Buffet'
    expect(page).to have_content 'Status do pedido: Avaliado'
    expect(page).to have_content 'Prazo final para confirmação e pagamento:'
    expect(page).to have_content 'Acréscimos:'
    expect(page).to have_content 'Descontos:'
    expect(page).to have_content "Valor do evento: #{order.final_price}"
    expect(page).to have_content "Valor total (com acréscimos e descontos): #{order.final_price + order.extra_tax - order.discount}"
    expect(page).to have_content 'Lembrando que há uma multa de 30 reais para cada hora passada.'
  end
