  ...
  context 'GET /api/v1/events/id/check_date?date=yyyy-mm-dd&guest_qtd=int' do
    ...
    it 'and the event are not available' do
      ...
      price = Price.create!(base_price: 3000, sp_base_price:4000, sp_additional_person:400,
                            additional_person:100, sp_extra_hour:60, extra_hour:50, event_id: event.id)
      customer = Customer.create!(email: 'cliente@email.com', password: 'password', name: 'cliente', cpf: '479.111.310-15')
      order = Order.create!(buffet_id: buffet.id, event_id: event.id, event_date: Date.new(2034, 12, 7), estimated_qtd: 30,
                            event_details: 'Esse evento deve possuir toda a temática com corações rosa', address: '', order_status: :confirmed,
                            final_price: 7000, extra_tax: 0, discount: 0, customer_id: customer.id, out_doors: false)
